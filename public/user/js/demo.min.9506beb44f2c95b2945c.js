! function() {
    ...
    angular.module("redeemar-app", ["app.constants", "ngRoute", "ngAnimate", "ngSanitize", "angular.mdl", "ml.chat", "ml.menu", "ml.svg-map", "ml.todo", "ui.select", "ngFileUpload", "ngWig", "pikaday", "ngPlaceholders", "ngTable", "uiGmapgoogle-maps", "gridshore.c3js.chart", "angularGrid", "LocalStorageModule" , "campaign-app", "inventory-app", "product-app", "logo-app", "repo-app", "promotion-app", "partnersetting-app", "video-app", "profile-app", "beacon-app"])
}(),
function() {
    "use strict";
    ...
    function() {
        ...
        function a(a, ra, h) {
           ...
           //$("#pp").html("uuu");
            ...
            //a.d_path=site_path;
            var site_path=$("#site_path").val();
            h.post("../user/userdetails")
            .success(function (data) {                
                a.ReedemerDetails=data;
                a.site_path=site_path;
                ra.default_logo_image = a.ReedemerDetails.logo_image;
            });
            ...
            //RouteScope Variables
            ra.EditOfferData = {};
            ra.campaignId = "";
        } 

        angular.module("redeemar-app").controller("MainController", ["$scope", "$rootScope", "$http", a])
...
function ($q,$rootScope,$location) {
    var interceptor = {
        'request':function(config){
          var token = angular.element('[name="csrf-token"]').attr('content');
        ...
        'response': function (response) {
            ...
            var token = angular.element('[name="csrf-token"]').attr('content');
            if(response.config.headers._token && response.config.headers._token != token){
              angular.element('[name="csrf-token"]').attr('content',response.config.headers._token)
            }
